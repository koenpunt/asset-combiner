...
JsCombine.prototype = {
  ...
  minify: function() {
    ...
    try {
      ...
      this.callback(null, this.filename.replace(/\.js$/, '.min.js'), output);
    } catch (e) {
      this.callback(e, this.filename, orig_code);
    }
  },
  ...
  print_coffee_error: function(error, file_name, file_contents) {
    ...
    if (line && line[1] && (line = parseInt(line[1]))) {
      ...
      console_error("Error compiling " + file_name + " \n" + error.message);
      index = 0;
      ...
      console_error("Error compiling " + file_name);
      return console_log("" + error.message + "\n");
    }
  }
};
